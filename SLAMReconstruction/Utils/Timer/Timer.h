...
#ifndef SLAMRECONSTRUCTION_TIMER_H
#define SLAMRECONSTRUCTION_TIMER_H
...
namespace gf {
    ...
    public:
        ...
        /**
        * duration between a time point in milliSeconds.
        * @return
        */
        inline unsigned int tickFromTimePoint(const std::chrono::system_clock::time_point &tt) {
            unsigned int start = std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::system_clock::now() - tt).count();
            return start;
        }
...
#endif //SLAMRECONSTRUCTION_TIMER_H
